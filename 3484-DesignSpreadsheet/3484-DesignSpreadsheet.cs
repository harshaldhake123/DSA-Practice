// Last updated: 9/19/2025, 10:07:52 PM
public class Spreadsheet {
    private readonly Dictionary<string,int> sheet;
    
    public Spreadsheet(int rows) {
        this.sheet=new Dictionary<string,int>();
    }
    
    public void SetCell(string cell, int value) {
        if(!sheet.ContainsKey(cell)){
            sheet.Add(cell,value);
        }else{
            sheet[cell]=value;
        }
    }
    ...
    private int GetCellValueFromReference(string cell){
        // find if its cell reference like A1 or just a number, eg.  6
